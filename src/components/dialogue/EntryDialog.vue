...
<template>
    <div v-if="action == 'select'" className="flex flex-col justify-between h-fit w-fit gap-8 p-4">
        <div className="grid grid-cols-3 gap-4 w-125" >
            <slot v-if="modalMode == 'education'" v-for="entry in account?.educationEntries" v-bind="entry" name="option"></slot>
            <slot v-else-if="modalMode == 'experience'" v-for="entry in account?.workExperiences" v-bind="entry" name="option"></slot>
            <slot v-else-if="modalMode == 'projects'" v-for="entry in account?.projects" v-bind="entry" name="option"></slot>
        </div>
